...
状态：`proposed_tested_not_approved`

本阶段固化未来 Claude reviewer 的凭据来源、模型和预算审批字段，并用纯本地 synthetic sentinel 验证秘密边界。它没有配置真实 API key、没有调用模型、没有产生费用，也没有启用现有 runner 或受限出站代理。正式评估启动门仍然关闭。
...
`reviewer/auth_budget/profile.json` 是待 Security Engineer 审批的机器可读提案：
...
模型和价格依据 2026-09-19 的 Anthropic 官方文档。正式运行前必须重新确认模型仍有效、价格未变，并把最终 profile SHA-256、批准人和 UTC 时间写入审批记录。
...
- `approval.approved` 为 `false`，批准人、时间和 profile hash 均为空；
- reviewer runtime 仍使用 `--network none` 和 `credential_injection: disabled`；
- egress proxy 仍未连接到 runner。
